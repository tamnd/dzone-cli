...
describes what users get. Pick a format with `-o`, or let dz choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dz <command> -o table   # aligned columns for reading
dz <command> -o jsonl   # one JSON object per line, for piping
dz <command> -o json    # a single JSON array
dz <command> -o csv     # spreadsheet friendly
dz <command> -o tsv     # tab-separated
dz <command> -o url     # just the URL column
dz <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dz <command> --fields id,title,url
```
...
```bash
dz <command>            # a table, because this is a terminal
dz <command> | wc -l    # JSONL, because this is a pipe
```
