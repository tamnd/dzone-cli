// Package cli builds the dz command tree on top of the dzone library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: dzone.DefaultConfig()}

	root := &cobra.Command{
		Use:   "dz",
		Short: "Browse DZone developer articles",
		Long: `dz reads DZone through its public RSS feeds.
No API key is required. Records are returned as table, JSON, JSONL, CSV, TSV, or URLs.

dz is an independent tool and is not affiliated with DZone or Progress.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = dzone.NewClient(a.cfg)
	return nil
}
